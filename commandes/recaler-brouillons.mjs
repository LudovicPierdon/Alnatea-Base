...
//   - besoin 0, ou produit dont le fournisseur n'est plus celui du brouillon → ligne à retirer à la main
//     (l'API ne sait ni supprimer une ligne ni la mettre à 0) ;
...
for (const b of bons.bonsOuverts.filter((b) => Number(b.status) === 0)) {
  ...
  for (const it of bons.lignesBon[b.id]) {
    ...
    const sidProduit = infoProduit(it.product_id)?.supplier_id;
    if (sidProduit && String(sidProduit) !== String(b.supplier_id)) { log(`  - ${sku} : fournisseur du produit = ${bons.nomFournisseur(sidProduit)}, ligne sur le brouillon ${bons.nomFournisseur(b.supplier_id)} : à retirer à la main`); E.actions.mauvaisFournisseur = (E.actions.mauvaisFournisseur || 0) + 1; continue; }
...
for (const pid of Object.keys(demande).sort()) {
  ...
  // « déjà dans un brouillon » ne compte que si ce brouillon est celui du fournisseur actuel du produit
  const dejaChezSonFournisseur = bons.bonsOuverts.some((b) => Number(b.status) === 0 && String(b.supplier_id) === String(info?.supplier_id) && (bons.lignesBon[b.id] || []).some((it) => String(it.product_id) === String(pid)));
  if (n <= 0 || dejaChezSonFournisseur) continue;
